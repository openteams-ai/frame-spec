...
5. Only content elements and `guards` compose. The values of `guidance` and its refinements combine according to rule 6. The values of `guards` accumulate: a Guard declared by any Frame in the composed set applies to the result. Every other element is the declaring Frame's own and MUST NOT be inherited from a composed Frame: the elements that describe the Frame or the version rather than its content, such as `identifier` and `maintainer`, and `composition` itself. That last one matters most and is the least obvious: a Frame that inherited its parent's `composition` would acquire the parent's references as its own, so resolving it again would pull grandparents in as direct parents. The set is not enumerated here because it is the complement of the first sentence, derivable from the element registry ([Section 10.2](#iana-elements)) as every element that neither refines `guidance` nor is `guards`; an enumeration would be a second statement of the same rule, and the two would drift.
